...
from core import PCAP_FILE, PROTOCOL_MAP, lock, global_state, playback_queue
                ...
                for ts, buf in pcap:
                    ...
                    if (ts - current_window_start >= 1.0) or (packet_count >= dynamic_limit):
                        ...
                        for (s_ip, d_ip), stats in window_flows.items():
                            ...
                            if pkts > 300:
                                window_alerts.append({"time": now_str, "icon": "⚠️", "type": "容量耗尽型 DDoS", "src_ip": s_ip, "target": target_str, "level": "high", "category": "DDoS攻击"})
                            
                            # 2. 端口扫描：1秒内这一个连接嗅探了 >= 3 种协议
                            elif len(protos) >= 3:
                                window_alerts.append({"time": now_str, "icon": "🔍", "type": "大范围端口探测", "src_ip": s_ip, "target": "多重协议", "level": "medium", "category": "端口扫描"})
                            
                            # 3. 蠕虫病毒：只要碰到 SMB 或 Telnet 协议，且包数 > 20 个
                            elif has_smb_telnet and pkts > 20:
                                window_alerts.append({"time": now_str, "icon": "🦠", "type": "疑似勒索蠕虫传播", "src_ip": s_ip, "target": "SMB/Telnet", "level": "high", "category": "蠕虫病毒"})
                            
                            # 4. 异常爬虫：Web 协议，且1秒内传输 > 1MB，包数还不多
                            elif is_web and byts > 1 * 1024 * 1024 and pkts <= 500:
                                window_alerts.append({"time": now_str, "icon": "🕷️", "type": "高频机器爬虫", "src_ip": s_ip, "target": "Web API", "level": "medium", "category": "异常爬虫"})
                            
                            # 5. 数据外泄：未知协议，且1秒内传输 > 1MB
                            elif is_unknown and byts > 1 * 1024 * 1024:
                                window_alerts.append({"time": now_str, "icon": "🔓", "type": "未知加密隧道外发", "src_ip": s_ip, "target": "Unknown", "level": "high", "category": "数据外泄"})

                        # --- 推送数据 ---
                        current_data = sorted([{"src_ip": k[0], "dst_ip": k[1], "bytes": v["bytes"], "packets": v["packets"], "protocols": list(v["protocols"])} for k, v in window_flows.items()], key=lambda x: x["bytes"], reverse=True)[:10]
                        topk_data = sorted([{"src_ip": k[0], "dst_ip": k[1], "bytes": v["bytes"], "packets": v["packets"], "protocols": list(v["protocols"])} for k, v in total_flows.items()], key=lambda x: x["bytes"], reverse=True)[:10]
