...
class ChatManager:
    ...
    async def connect(self, ws: WebSocket):
        ...
        self.sessions[ws] = {
            "history": deque(maxlen=MAX_CHAT_TURNS * 2),
    ...
    def get_session(self, ws: WebSocket) -> dict:
        return self.sessions.setdefault(ws, {
            "history": deque(maxlen=MAX_CHAT_TURNS * 2),
            "connected_at": time.time(),
            "last_active_at": time.time(),
        })

    def append_history(self, ws: WebSocket, role: str, content: str):
        session = self.get_session(ws)
        session["history"].append({"role": role, "content": content})
        session["last_active_at"] = time.time()

    def get_history(self, ws: WebSocket) -> list:
        session = self.get_session(ws)
